...
const ImageModal = ({ isOpen, images, initialSlide, onClose }) => {
  ...
  return (
    ...
    >
      ...
      <div
        ref={modalRef}
        className=" h-[30rem] items-center flex modal-container px-7 py-4 bg-white w-5/6 md:max-w-xl mx-auto rounded-lg z-50 relative modal-enter"
      >
        <span
          ...
          {images.map((img, i) => (
            <SwiperSlide className="" key={i}>
              <img
                src={img}
                alt="ecommerce"
                className=" rounded-[1.5rem] w-full h-[20rem] md:w-full md:h-[30rem] py-4 "
              />
            </SwiperSlide>
          ))}
        </Swiper>
